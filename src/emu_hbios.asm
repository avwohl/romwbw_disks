	.z80
;
; The RomWBW version this bank 0 claims is NOT written here.  romwbw_ver.inc
; is generated by tools/build_rom.sh from versions/<ver>/version.json, so one
; source builds a ROM for any RomWBW release and the two copies below can
; never drift from each other or from the published catalog.
; See docs/INTERFACE_V0.md.
;
	include	romwbw_ver.inc
;
;==================================================================================================
...
CB_VERSION:	db	RMV_VER		; (major<<4)|minor (offset 0x05)
		db	RMV_UPD		; (update<<4)|patch (offset 0x06)
...
; Offset 0x00: Ident block
	...
	db	RMV_VER			; (major<<4)|minor
	db	RMV_UPD			; (update<<4)|patch
...
;
; 0xFFFC and 0xFFFE both hold the ident pointer ON PURPOSE.  Upstream moved
; HB_IDENT from HBX_XFCFNS+14 (0xFFFE) to HBX_XFCFNS+12 (0xFFFC) in v3.6.0
; (Source/HBIOS/hbios.inc).  Writing both is what lets one proxy serve both
; releases.  Do not remove the apparent duplicate - a guest built against the
; other release would read a zero and follow it.
